...
uint64_t unwrap(WrappingInt32 n, WrappingInt32 isn, uint64_t checkpoint) {
    // the diff between two seqnos is same as the diff of their corresponding absolute seqnos
    WrappingInt32 wrapped_checkpoint = wrap(checkpoint, isn);
    uint32_t diff = n - wrapped_checkpoint;
    // if checkpoint < diff, don't cast diff to signed int
    if (checkpoint < diff) {
