...
class TCPSegment {
  ...
  public:
    ...
    ParseResult parse(const Buffer &buffer, uint32_t datagram_layer_checksum = 0);

    //! \brief Serialize the segment to a string
    [[nodiscard]] BufferList serialize(uint32_t datagram_layer_checksum = 0) const;

    //! \name Accessors
    //!@{
    [[nodiscard]] const TCPHeader &header() const { return _header; }
    TCPHeader &header() { return _header; }

    [[nodiscard]] const Buffer &payload() const { return _payload; }
    ...
    [[nodiscard]] size_t length_in_sequence_space() const;
};
